	...
	if title_column == []:
		...
		for i in range(top):
			...
			if type == 'movie':
				...
				movie = dict(title = title, year = year, duration = duration, poster = poster, trailer = trailer, storyline = storyline,
							director = director, stars = stars)
				movies.append(movie)
			elif type == 'tv':
				creator = soup.select('.credit_summary_item span[itemprop=creator]')[0].getText().strip()
				tv_show = dict(title = title, year = year, duration = duration, poster = poster, trailer = trailer, storyline = storyline,
...
if __name__ == '__main__':
	...
	with open('tv_shows.json', 'w') as t:
